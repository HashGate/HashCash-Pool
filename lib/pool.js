...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            var login = params.login.split(config.poolServer.fixedDiff.addressSeparator)[0];
            if (!login){
                log('error', logSystem, 'Miner missing login.',[difficulty]);
                sendReply('missing login');
                return;
            }
			//var Address = login.indexOf(config.poolServer.fixedDiff.addressSeparator);
            var difficulty = portData.difficulty;
            var noRetarget = false;
            /* if(config.poolServer.fixedDiff.enabled) {
                var fixedDiffCharPos = params.login.split(config.poolServer.fixedDiff.addressSeparator)[0];
            ...
            log('warn',logSystem,'Address: '+params.login.split(config.poolServer.fixedDiff.addressSeparator)[0]);
            if (!utils.isValidAddress(params.login.split(config.poolServer.fixedDiff.addressSeparator)[0], config.poolServer.poolAddress[0])){
